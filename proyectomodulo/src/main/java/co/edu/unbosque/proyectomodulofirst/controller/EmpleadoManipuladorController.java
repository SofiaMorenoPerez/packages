...
public class EmpleadoManipuladorController {
    ...
    public ResponseEntity<String> crearManipulador(@RequestParam String nombre, @RequestParam int edad,
        ...
        if (status == 0) {
            return new ResponseEntity<>("Manipulador creado con exito", HttpStatus.CREATED);
        } else if (status == 1) {
            ...
            return new ResponseEntity<>("El tipo de paquete ingresado no es valido", HttpStatus.BAD_REQUEST);
        } else {
            return new ResponseEntity<>("Error al crear manipulador", HttpStatus.INTERNAL_SERVER_ERROR);
        }
    }
    ...
    public ResponseEntity<String> actualizarManipulador(@RequestParam Long id, @RequestParam String nombre,
        ...
        if (status == 0) {
            return new ResponseEntity<>("Manipulador actualizado con exito", HttpStatus.OK);
        } else if (status == 1) {
            return new ResponseEntity<>("Manipulador no encontrado con id: " + id, HttpStatus.NOT_FOUND);
        } else if (status == 2) {
            return new ResponseEntity<>("La fecha de inicio es obligatoria", HttpStatus.BAD_REQUEST);
        } else if (status == 3) {
            return new ResponseEntity<>("El nombre ingresado no es valido", HttpStatus.BAD_REQUEST);
            ...
            return new ResponseEntity<>("Error al actualizar manipulador", HttpStatus.INTERNAL_SERVER_ERROR);
        }
    }
    ...
    public ResponseEntity<String> eliminarManipulador(@RequestParam Long id) {
        ...
        if (status == 0) {
            return new ResponseEntity<>("Manipulador eliminado con exito", HttpStatus.OK);
        } else if (status == 1) {
            return new ResponseEntity<>("No se puede eliminar el manipulador porque tiene paquetes asignados", HttpStatus.BAD_REQUEST);
        } else if (status == 2) {
            return new ResponseEntity<>("Manipulador no encontrado con id: " + id, HttpStatus.NOT_FOUND);
        } else {
            return new ResponseEntity<>("Error al eliminar manipulador", HttpStatus.INTERNAL_SERVER_ERROR);
        }
    }
}
